...
def main():
    ...
    config_path = os.path.join(ROOT, "config", "spectra", "spectra_qwen_config.json")
    ...
    from training.spectra_sft.train_spectra import _patch_qwen3_participation_guard
    _patch_qwen3_participation_guard()

    # 2) 토크나이저
    from training.spectra_sft.train_spectra import setup_tokenizer
    if rank == 0:
    ...
    ds_config_path = os.path.join(ROOT, "config", "spectra", "deepspeed_zero2.json")
