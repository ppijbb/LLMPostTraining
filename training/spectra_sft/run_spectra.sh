...
# Default config file and GPU count (configs under repo config/spectra/)
CONFIG_FILE="${1:-$PROJECT_ROOT/config/spectra/$CONFIG_FILE_NAME}"
...
if [ ! -f "$CONFIG_FILE" ]; then
    ...
    find "$PROJECT_ROOT/config/spectra" -name "*.json" -type f 2>/dev/null | sort || echo "  No config files found"
    exit 1
fi
...
if [ "$RUN_BASELINE" = "1" ]; then
    ...
    --config_file $PROJECT_ROOT/config/spectra/accelerate.yaml \
