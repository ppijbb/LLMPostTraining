#!/usr/bin/env python3
"""
Seqorth Light Training Script
Simplified version of train_seqorth.py - exactly matching dataset loading and model options.
"""
...
# Expect PYTHONPATH to include repo root (workspace_root)
...
from models.seqorth_model import SeqorthForConditionalGeneration, SeqorthConfig, SeqorthMoE
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Seqorth Light Training")
    ...
    # setup tokenizer/processor matching train_seqorth.py
    ...
    # Dataset setup EXACTLY matching train_seqorth.py
    ...
    seqorth_params = model_config.get("seqorth_params", {})
    ...
    logger.info(f"🤖 Loading Seqorth model on rank {rank} with {model_dtype}...")
    ...
    if model_config.get("deepspeed_config"):
        ...
        # Match train_seqorth.py: patch "auto" to 1 for zero.Init() validation, then calculate actual value
    ...
    # Load model with memory-efficient settings
    model = SeqorthForConditionalGeneration.from_pretrained(
        model_config["model_name_or_path"],
        config=config,
        trust_remote_code=model_config.get("trust_remote_code", True),
    ...
    # PEFT / LoRA setup EXACTLY matching train_seqorth.py
    ...
    # Expert training setup matching train_seqorth.py (lines 1681-1736)
    ...
    # Training Arguments - match train_seqorth.py exactly
    training_args = SFTConfig(
        ...
        run_name=training_config.get("run_name", "seqorth_light"),
        logging_steps=1,
        save_strategy="no",
