...
def run_information_processing_comparison(
    ...
    Seqorth vs Dense vs Baseline MoE
    """
    print("="*60)
    print("Information Processing Quality Comparison")
    print("="*60)
    
    tokenizer = AutoTokenizer.from_pretrained(seqorth_model_path)
    
    # Seqorth 모델
    seqorth_model = AutoModelForCausalLM.from_pretrained(seqorth_model_path)
    seqorth_model = seqorth_model.to(device)
    
    results = {'Seqorth': {}, 'dense': {}, 'baseline_moe': {}}
    
    # Seqorth 평가
    print("\nEvaluating Seqorth model...")
    for task_name, task_data in eval_datasets.items():
        if task_name == 'language_modeling':
            score = evaluate_model_perplexity(seqorth_model, tokenizer, task_data, device)
            results['Seqorth'][task_name] = score
    ...
    if 'dense' in results and 'language_modeling' in results['dense']:
        seqorth_ppl = results['Seqorth']['language_modeling']['perplexity']
        dense_ppl = results['dense']['language_modeling']['perplexity']
        improvement = dense_ppl - seqorth_ppl  # 낮을수록 좋으므로
        relative_improvement = (improvement / dense_ppl) * 100
        
        comparison['vs_dense'] = {
            'seqorth_ppl': seqorth_ppl,
            'dense_ppl': dense_ppl,
        ...
        print(f"\nSeqorth vs Dense:")
        print(f"  Seqorth PPL: {seqorth_ppl:.4f}")
    ...
    if 'baseline_moe' in results and 'language_modeling' in results['baseline_moe']:
        ...
        improvement = baseline_ppl - seqorth_ppl
...
def main():
    parser = argparse.ArgumentParser(description="Seqorth MoE Validation Scripts")
    parser.add_argument("--task", type=str, required=True, choices=['ablation', 'comparison', 'both'],
                       help="Validation task to run")
    parser.add_argument("--seqorth_model", type=str, required=True,
                       help="Path to Seqorth model")
    ...
    if args.task in ['comparison', 'both']:
        run_information_processing_comparison(
            seqorth_model_path=args.seqorth_model,
