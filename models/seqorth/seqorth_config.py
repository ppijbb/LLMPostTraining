...
class SeqorthTextConfig(PretrainedConfig):
    r"""
    This is the configuration class to store the configuration of a [`SeqorthTextModel`]. It is used to instantiate an SeqorthText
    model according to the specified arguments, defining the model architecture. Instantiating a configuration with the
    defaults will yield a similar configuration to that of the SeqorthText-7B.
    e.g. [google/seqorth_text-7b](https://huggingface.co/google/seqorth_text-7b)
    Configuration objects inherit from [`PretrainedConfig`] and can be used to control the model outputs. Read the
    documentation from [`PretrainedConfig`] for more information.
    Args:
        vocab_size (`int`, *optional*, defaults to 262208):
            Vocabulary size of the SeqorthText model. Defines the number of different tokens that can be represented by the
            `inputs_ids` passed when calling [`SeqorthTextModel`]
    ...
    ```python
    >>> from transformers import SeqorthTextModel, SeqorthTextConfig
    >>> # Initializing a SeqorthText seqorth_text-7b style configuration
    >>> configuration = SeqorthTextConfig()
    >>> # Initializing a model from the seqorth_text-7b style configuration
    >>> model = SeqorthTextModel(configuration)
...
class SeqorthConfig(PretrainedConfig):
    r"""
    This is the configuration class to store the configuration of a [`SeqorthForConditionalGeneration`]. It is used to instantiate an
    SeqorthForConditionalGeneration according to the specified arguments, defining the model architecture. Instantiating a configuration
    with the defaults will yield a similar configuration to that of the PaliGemma-2B.

    e.g. [google/seqorth-4b](https://huggingface.co/google/seqorth-4b)

    Configuration objects inherit from [`PretrainedConfig`] and can be used to control the model outputs. Read the
    documentation from [`PretrainedConfig`] for more information.

    Args:
        text_config (`Union[SeqorthTextConfig, dict]`, *optional*):
    ...
    ```python
    >>> from transformers import SeqorthForConditionalGeneration, SeqorthConfig, SiglipVisionConfig, SeqorthTextConfig

    >>> # Initializing a Siglip-like vision config
    >>> vision_config = SiglipVisionConfig()

    >>> # Initializing a Seqorth Text config
    >>> text_config = SeqorthTextConfig()

    >>> # Initializing a Seqorth seqorth-4b style configuration
    >>> configuration = SeqorthConfig(vision_config, text_config)

    >>> # Initializing a model from the seqorth-4b style configuration
    >>> model = SeqorthTextConfig(configuration)
    ...
    def __init__(
        self,
        text_config: Optional[Union[SeqorthTextConfig, Dict[str, Any]]] = None,
        ...
        **kwargs,
        ...
        if text_config is None:
            text_config = SeqorthTextConfig()
            logger.info("text_config is None, using default SeqorthTextConfig vision config.")
        elif isinstance(text_config, dict):
            text_config.update(kwargs)
            text_config = SeqorthTextConfig(**text_config)
        ...
        if isinstance(vision_config, dict):
            ...
            vision_config = SiglipVisionConfig()
            logger.info(
                "vision_config is None or incompatible with SeqorthVisionConfig intialization. Seqorth will be limited "
                "to text tasks."
            )
...
__all__ = ["SeqorthConfig", "SeqorthTextConfig"]
